...
| F16 | **The `diffusion` wall loss excludes the generic charger ions; no other loss does.** That branch alone loops `for iclus = 1 .. $max_cluster` (the real clusters) where every other loss branch loops to `$max_cluster_number`, which includes the two generic ions. The fixture has exactly 52 nonzero entries, zero at the generic-ion slots. Reproduced. | Perl `:7325` vs e.g. `:7159` | `generic_ion_mask` | excluded |
