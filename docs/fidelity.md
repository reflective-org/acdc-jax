...
| F7 | **Hydrate distributions whose normalised sum is not above 0.99 are silently discarded** and the cluster treated as dry (Perl `:2432-2440`). Since the sum of normalised weights is 1 by construction, the check can only fail on a non-finite distribution (an overflowing Boltzmann factor gives NaN, and `NaN > 0.99` is false) — so in practice a hydrate that is *too* stable makes upstream quietly ignore all of that cluster's hydrates. Reproduced, but decided from the EXPONENT rather than by forming the overflow: normalising in linear space gives a finite answer through the final `where` while leaving reverse mode crossing an inf/inf, so `jax.grad` returned NaN. The port normalises shifted by the per-cluster maximum -- the same ratios -- and takes the dry fallback when any exponent exceeds `log(float64 max)`, which is exactly when upstream's sum overflows. | Perl `:2432-2440` | `hydrate_discard_threshold` | `0.99` |
